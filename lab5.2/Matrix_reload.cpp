...
void Matrix::input() {
	...
	if (this->elem != nullptr) // если в массиве "что-то есть", то удаляем всез значения.
	{
		delete[] this->elem;
	}
	this->elem = new double[this->size_row * this->size_col]; //создание массива
	for (int i = 0; i < this->size_row * this->size_col; i++) {
		std::cin >> *&this->elem[i];
	}
}
// перегрузка функции input (добавили аргументы i, j)
...
// перегрузка функции input (добавили аргументы i, j, массив arr)
...
double Matrix::get_elem(int i, int j) { // получение элемента по индексу
	return this->elem[i * this->size_col + j];
}
void Matrix::mult_by_num(double num) { //  умножить на число матрицу
	for (int i = 0; i < this->size_row; i++) {
		for (int j = 0; j < this->size_col; j++) {
...
bool Matrix::sum(const Matrix* mat2) { // сложить две матрицы
	if ((this->size_col == mat2->size_col) and (this->size_row == mat2->size_row)) { // проверка на тождественность столбцов и строк соответсвенно
		for (int i = 0; i < this->size_row; i++) {
			for (int j = 0; j < this->size_col; j++) {
...
// ПЕРЕЗАГРУЗКА
bool Matrix::sum(const double* arr, int row, int col) { // перезагрузка функции сложения двух матриц
...
bool Matrix::mult(const Matrix* mat2) { // умножение матрицы на матрицы
